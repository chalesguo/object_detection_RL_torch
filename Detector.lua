...
function Detector:detectFgBg(input)
  ...
  return matches,outputs[#outputs]
end
...
function Detector:detectObjects(input,matches)
  ...
  if #matches > 0 then
    ...
    if false then
      winners = self:evaluate(coutputs, candidatesup)
    else
      winners = self:evaluatewithNMS(coutputs, candidatesup)
    end
  end
...
function Detector:detect(input)
  local matches,output = self:detectFgBg(input)

  if self.mode == 'onlyPnet' then
    return matches
  else
    return self:detectObjects(output,matches)
  end
end
