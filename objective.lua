...
function create_objective(model, weights, gradient, batch_iterator, stats, pnet_confusion, cnet_confusion, mode, pnet_copy)
  ...
  local function lossAndGradient(w)
    ...
    -- enable dropouts
    if pnet_copy then
      pnet_copy:training() --pnet_copy:evaluate()
    end
    pnet:training()
    cnet:training()
    ...
    for i,x in ipairs(batch) do
      ...
      if pnet_copy then
        ...
        cleanAnchors(n,  outputs_c)
        cleanAnchors(p,  outputs_c)
      end

      -- process positive set
      local lambda = 10    -- weight for box proposal regression
      ...
      if mode ~= 'onlyPnet' then
        ...
        if #roi_pool_state > 0 then
          ...
          if #p < #roi_pool_state then
            ...
            crtarget[{{#p + 1, #roi_pool_state}, {}}]:zero()
          end
          ...
          local loss = cnll:forward(ccout, cctarget:cuda())
          ccls_loss = ccls_loss + loss
          local ccdelta = cnll:backward(ccout, cctarget:cuda())
